...
async def lego(event):
 ...
 if event.sender_id == OWNER_ID:
    ...
    if not quew:
       ...
       pass
 await event.reply('Making Your Logo Please Wait..🖼️!')
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DaisyX/resources/d8klayt-b5ff4060-2c98-4b17-a071-ed88af832d16.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./DaisyX/resources/Chopsic.otf", 50)
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="This Logo Made By @szkingster_bot ....")
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DaisyX/resources/photo_2021-08-21_23-18-11.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./DaisyX/resources/Maghrib.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./DaisyX/resources/Chopsic.otf", 330)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DaisyX/resources/pandabg.png')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./DaisyX/resources/font.otf", 100)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
